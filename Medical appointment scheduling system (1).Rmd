...
slots <- read_csv("C:/Users/Jaehui Park/Desktop/Portfolio/1) Data/1) Medical Appointment Scheduling system/slots.csv")

patients <- read_csv("C:/Users/Jaehui Park/Desktop/Portfolio/1) Data/1) Medical Appointment Scheduling system/patients.csv")


appointments <- read_csv("C:/Users/Jaehui Park/Desktop/Portfolio/1) Data/1) Medical Appointment Scheduling system/appointments.csv")
...
# Verify uniqueness of primary keys in core tables
...
```{r}
# 1. Data preprocessing and identify slot recovery
recovery_analysis <- past_appointments %>%
  # Extract necessary columns and create time slots
  mutate(
    ...
    # Fix the order of weekdays for consistent plotting
    day_of_week = factor(day_of_week, 
                         levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"))
  ) %>%
  # Determine whether cancelled slots were refilled
  ...
  ) %>% # <--- 여기서 mutate를 닫고 파이프를 써야 합니다.
  ungroup()

# 2. Summarize recovery rate by weekday and time slot
recovery_summary <- recovery_analysis %>%
  filter(is_cancelled == TRUE) %>%
  group_by(day_of_week, time_slot) %>%
  summarise(
    total_cancelled = n_distinct(slot_id),                  # Total cancelled slots
    refilled_count = n_distinct(slot_id[is_refilled == TRUE]), # Number of cancelled slots that were refilled
    recovery_rate = (refilled_count / total_cancelled) * 100, # Recovery rate in %
    .groups = 'drop'
  ) %>%
  # Ensure time_slot factor order for visualization
  mutate(time_slot = factor(time_slot, 
                            levels = c("Evening (17-)", 
                                       "Afternoon (12-17)", 
                                       "Morning (08-12)")))

# 3. Heatmap visualization: Recovery Rate
ggplot(recovery_summary, aes(x = day_of_week, y = time_slot, fill = recovery_rate)) +
  geom_tile(color = "white", size = 0.5) +
  scale_fill_gradient(low = "#FFF5F0", high = "#CB181D") +
  geom_text(aes(label = paste0(round(recovery_rate, 1), "%")), 
            size = 4, fontface = "bold") +
  labs(
    title = "Slot Recovery Rate by Weekday and Time Slot",
    subtitle = "Proportion of cancelled slots that were filled by other patients\n(Identify 'dead slots' that are rarely reused)",
    x = "Weekday", y = "Time Slot", fill = "Recovery Rate (%)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 15, face = "bold"),
    plot.subtitle = element_text(size = 10, lineheight = 1.1, color = "grey30"),
...
BA의 최종 운영 제안: "3-7일의 법칙"
예약 확인 콜 최적화: 재활용률이 가장 높은 진료 3~7일 전에 환자들에게 예약 확정 알림을 보내고, 취소 의사가 있다면 이 시점에 확정받아 대체 환자를 찾아야 합니다.

단기 취소 패널티 및 보상: 재활용률이 낮은 1~2일 전 취소를 줄이기 위해 노쇼 방지 예약금을 도입하거나, 반대로 이 시점에 예약하는 환자에게는 혜택을 주는 '라스트 미닛(Last-minute)' 예약을 활성화해야 합니다.

대기자 시스템(Waitlist) 자동화: 시간대와 요일에 상관없이 실패율이 높은 것은 수동 관리에 한계가 있다는 뜻입니다. 취소가 발생하는 즉시 대기 환자에게 자동 푸시 알림을 보내는 시스템 도입이 시급합니다.
...
1) 특정 보험사?
2) 연령대별로?
비즈니스 가치: 빈 슬롯 발생 시 즉시 알림을 보낼 타겟 대상(당일 예약 선호군) 정의 가능


-재활용된 슬롯에 '나중에 들어와서 진료를 받은(attended)' 환자들의 특성을 분석
```{r}
...
이번엔 이전 진료경험이 있는 고객 특징 - return customer feature 분석
...
기초탐색
-loyal VS churned 고객 특성 비교


Loyal VS Churned 고객 데이터셋
```{r}
...
1. 연도별 충성도(Loyal Rate) 흐름 분석
2015년 - 2019년 (황금기): 히트맵이 전반적으로 **푸른색(Loyal 비중 90% 이상)**을 띠고 있습니다. 이 시기에는 요일이나 시간대에 상관없이 환자들이 병원을 믿고 다시 찾는 '충성 고객' 중심의 운영이 매우 잘 이루어졌음을 알 수 있습니다.

2021년 - 2023년 (위기기): 2021년부  터 히트맵에 **붉은색(Loyal 비중 75%~80%)**이 급격히 늘어납니다. 이는 충성 고객들이 이탈하고 신규 환자(또는 일회성 환자) 비중이 높아졌거나, 기존 고객들이 병원에 실망하여 'Churned(이탈)' 상태로 돌아섰음을 의미합니다.

2024년 (회복 조짐): 다시 푸른색이 보이기 시작하며 충성도가 반등하는 모습을 보입니다.

2. 시간대별 '직원 성과' 추론 (Proxy Analysis)
질문하신 것처럼 특정 시간대에 Loyal 비중이 높다면, 해당 시간대 근무자의 환자 응대 서비스나 진료 만족도가 높을 가능성이 매우 큽니다.

Afternoon (12-17)의 붕괴: 2022년과 2023년 데이터를 보면 Afternoon 시간대가 유독 붉게 변합니다.

분석: 우리가 앞서 분석했던 '오후 시간대 대기 시간 폭증(70분)' 현상이 이 시기에 심화되었을 가능성이 큽니다.

결론: 긴 대기 시간에 지친 충성 고객들이 이 시기에 대거 이탈했음을 시사합니다. 즉, 직원의 역량 문제라기보다 **'운영 시스템의 병목'**이 고객 충성도를 깎아먹은 주범일 수 있습니다.

Evening (17-)의 상대적 선전: 2021년~2022년의 Evening 시간대는 Afternoon보다 푸른 빛을 더 유지하고 있습니다. 퇴근 후 진료를 보는 이 시간대 담당 인력이 환자 관리를 더 세심하게 잘했을 확률이 높습니다.

3. 비즈니스 가치: '구원 투수' 인력 배치 전략
이 히트맵을 바탕으로 다음과 같은 인력 운영 전략을 세울 수 있습니다.

High-Loyal 인력 벤치마킹: 2024년에 다시 충성도를 끌어올린 Morning(오전) 근무자들의 환자 응대 매뉴얼을 전사적으로 확산시켜야 합니다.

Wednesday(수요일) 집중 관리: 2023년 수요일 Afternoon의 충성도가 최저점을 찍었습니다. 앞서 확인했듯 재활용률도 낮고 혼잡도도 높은 이 시간대에는 가장 숙련된 베테랑 직원을 배치하여 고객 이탈을 막아야 합니다.
...
하지만 Loyal 이 줄어든 이유가 신규 고객 유입이 늘어서라면?
```{r}
...
1. 시간대별 '환자 밀도' 분석 (Bottle-neck Discovery)
단순히 예약 인원만 보는 게 아니라, 실제 머무는 시간을 계산해 대기실에 동시에 몇 명이 머무는지 추정해야 합니다.

방법: appointment_time (시작) + waiting_time + appointment_duration (종료)을 계산하여, 10분 단위로 병원 내에 체류 중인 환자 수를 카운트합니다.

활용: "오전 10시 30분에는 평균적으로 대기실에 15명이 앉아 있다"는 수치를 도출합니다. 만약 의자가 10개뿐이라면, 이때가 바로 혼잡 위험 시간입니다.

2. '혼잡 유발자'와 '혼잡 피해자' 구분
연령대별 분석 결과와 연결하여 인력 배치를 최적화합니다.

혼잡 유발자(진료 시간이 긴 그룹): 우리 데이터에서 55-79세는 진료 시간이 길고 비중도 높습니다. 이들이 몰리는 시간대에는 대기실 회전율이 급격히 떨어집니다.

혼잡 피해자(이탈에 민감한 그룹): 15-24세는 혼잡도가 높아 대기가 길어지면 바로 이탈(Churn)로 이어집니다.

전략: 15-24세 예약은 최대한 '혼잡 유발 그룹'과 겹치지 않게 분산 배치하거나, 이들이 올 때는 행정 처리를 전담할 인력을 추가 배치합니다.
...
저녁에 대기실 혼잡도가 올라가는데 그 이유 예측
- 오전부터 쌓인 고객이 눈덩이처럼 불어나서?
- 오후 시간대에 유독 진료시간이 긴 연령대 (50-70대)가 몰려서?
- 예약을 capacity 보다 많이 받아서?
...
시각화 보정: avg_waiting과 avg_duration만 따로 떼어낸 그래프 필요요. 현재는 patient_count의 숫자가 너무 커서 대기 시간의 미세한 변화(예: 10분에서 20분으로 증가)가 그래프에 반영되지 않는 부분 반영
...
1. 레드 라인(평균 대기시간)의 무서운 우상향
현상: 오전 8시에는 약 15분 내외였던 대기 시간이 시간이 흐를수록 직선적으로 상승하여, 오후 17시에는 70분 가깝게 치솟습니다.

해석: 이는 전형적인 **'눈덩이 효과(Snowball Effect)'**입니다. 환자 수(patient_count)는 전 시간대 약 11,000명 수준으로 일정한데 대기 시간만 늘어난다는 것은, 시스템이 환자를 처리하는 속도보다 예약이 들어오는 속도가 미세하게 빨라 지연이 매시간 복리로 쌓이고 있음을 의미합니다.

2. 그린 라인(평균 진료시간)의 안정성
현상: 진료 시간은 전 시간대 약 20분 내외로 매우 일정하게 유지됩니다.

해석: 의료진은 매시간 성실하고 일정하게 진료를 보고 있습니다. 즉, 혼잡의 원인은 "의사가 진료를 늦게 봐서"가 아니라, "예약 슬롯 간격이 너무 촘촘해서" 발생하는 시스템 설계의 문제입니다.

3. 히트맵(image_c434f8.png)과의 연결고리 확인
오후 15시~17시의 혼잡 지수가 왜 125,000 이상으로 짙은 보라색이었는지 이제 설명이 됩니다. 환자 수는 오전과 같지만, 개별 환자가 대기실에 머무는 시간(Waiting Time)이 4배 이상(15분 → 70분) 늘어났기 때문에 대기실 밀도가 폭발한 것입니다.
...
혼잡도와 대기시간의 역설
오후 시간대 과부하: 환자 수는 모든 시간대에 1,700명 선으로 일정함에도 불구하고, **혼잡 지수(Load)**는 오후 3시(15시)를 기점으로 급격히 상승하여 오후 5시에 최고조에 달합니다.

누적 지연 효과: 이는 환자가 더 많이 와서가 아니라, 오전부터 처리되지 못한 대기 인원이 오후에 쌓이는 **'병목 현상(Snowball Effect)'**이 발생하고 있다는 증거입니다.

예약 최적화시간당 2,240건의 예약 캡(Cap)을 실제 처리 능력인 1,800건 수준으로 하향 조정하여 노쇼 관리 강화인력 탄력 운용

혼잡 지수가 폭발하는 오후 3시~5시에 행정 및 지원 인력을 금요일 수준으로 보강

3. 요일별 운영 효율성 차이
금요일의 높은 효율성: 금요일은 실제 진료 환자가 가장 많음에도 불구하고(17,050명), 평균 대기시간은 42.0분으로 모든 요일 중 가장 짧습니다.

목요일의 병목 심화: 반면 목요일은 환자 수가 금요일보다 적음에도 대기시간이 44.9분으로 가장 깁니다. 목요일의 인력 배치나 진료 프로세스에 비효율적인 구간이 있을 가능성이 큽니다.


1. 개별 환자가 아닌 '전체 대기실'의 관점우리 병원은 시간당 약 1,700명의 환자를 진료합니다. 이 인원에게 2.9분의 차이를 대입하면 수치가 완전히 달라집니다.시간당 절약되는 총 대기 시간: $1,700 \text{명} \times 2.9 \text{분} \approx 4,930 \text{분}$ (약 82시간)하루 전체(10시간)로 환산: 하루에 약 820시간의 대기 시간이 금요일에는 목요일보다 덜 발생하고 있는 셈입니다.즉, 대기실 의자 하나에 머무는 시간이 아주 조금씩 짧아지면서 대기실 전체의 '혼잡 밀도'를 낮추는 효과는 2.9분 그 이상입니다.


1. 연령대별 예약 요일 및 시간대 집중도 분석 코드
이 코드는 연령대를 그룹화하여 특정 시간대와 요일에 어떤 층이 더 많이 나타나는지 **비중(Percentage)**으로 보여줍니다. -> 향후 프로모션을 통해 연령대별로 예약 시간 유도 가능능
...
고령층 55-79세 예약을 아침으로 유도 마케팅 시뮬레이션
...
ggplot(comparison_sim, aes(x = hour, y = congestion_index, color = type, group = type)) +
  ...
  theme_minimal()
```
그래도 2시에 잠깐 쉬어가는 시간이 좀 생기지만 여전히 5시에 병목현상
...
17시로 인력을투입하는건 너무 늦는듯, 그전에 미리 투입해야 대기 막을수 있음
...
14시 전진배치 + 마케팅으로 고령층 55-79 아침예약 유도 [월-금]
...
대기실 혼잡도를 조정하기 위해서는 손님을 옮길게 아니라 오전에 여유로운 시간대의 인력을 오후로 옮기던가 아니면 오후에 학생등 아르바이트를 쓰는것이 효율 적!




Tableau Dashboard 제작을 위한 CSV 파일 저장
